...
const DateRangePicker: React.FC<DateRangePickerProps> = ({
    ...
    const [selectedDateRange, setselectedDateRange] = useState(
        StaticDateRangeData[selectedOptionIndex || 0]
    );
    ...
    useEffect(() => {
        if (selectedDateRange.dateDiff) {
            ...
            const dateString = convertDateToString(
                updatedDates as [Date | null, Date | null]
            );
    ...
    const convertDateToString = (dates: [Date | null, Date | null]) => {
        ...
        if (endDate) {
            ...
            )} - ${format(endDate || Date(), dateFormat || "dd/MM/yyyy")}`;
            return dateString;
        } else {
    ...
    const calculateDates = (diff: number | null, dateLabel: string | null) => {
        ...
        if (diff) {
            ...
            return compareAsc(currDate, nextDate)
                ? [nextDate, currDate]
                : [currDate, nextDate];
        } else if (dateLabel) {
    ...
    const dropdownValueTemplate = (option: any) => {
        return (
            <div className="flex">
                ...
                ></i>
                <i
                    className="pi pi-calendar ml-2 flex-end"
                    style={{ fontSize: "1rem" }}
                ></i>
            </div>
        );
    ...
    return (
        <PrimeReactProvider>
            ...
            <Dropdown
                ...
                onChange={(e) => handleSelectedDateOption(e.value)}
                options={StaticDateRangeData}
                ...
                dropdownIcon="none"
            /><br></br>
            {selectedDateRange.optionLabel === "Custom Range" &&
                isDatePickerVisible && (
                    <>
                        <Calendar
                            dateFormat="dd/mm/yy"
